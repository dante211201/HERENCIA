...
public class Herencia {
    ...
    private static Dog2[] insertDog() {
        ...
        ///Asigna los nombres 
        for(int i=0; i < dogs.length ; i++){
            System.out.println("INGRESA EL NOMBRE DE TU PERRO " + (i+1));
            String nombre = teclado.nextLine();
            names[i]=nombre;
        }
        //Asigna el Color
        for(int i=0; i < dogs.length ; i++){
            System.out.println("INGRESA EL COLOR DE TU PERRO " + (i+1));
            String color= teclado.nextLine();
            colours[i]=color;
        }
        //Asinga el peso
        for(int i=0; i < dogs.length ; i++){
            System.out.println("INGRESA EL PESO DE TU PERRO " + (i+1));
            Double peso= teclado.nextDouble();
            weight[i]=peso;
        }
        //Asignar la porción
        for(int i=0; i < dogs.length ; i++){
            System.out.println("INGRESA LA PROPORCION DE TU PERRO " + (i+1));
